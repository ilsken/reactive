...
exports.interpolationProps = function(str) {
  ...
  while (m = adapter.interpolation.regex.exec(str)) {
    var expr = m[1];
...
exports.interpolate = function(str, fn) {
  var result, open = 0, index = 0, chunks = [];
  while((result = adapter.interpolation.regex.exec(str))) {
    var chunk, char = result[0];
      if (adapter.interpolation.start === char) {
        if (!open) {
          chunks.push(str.substr(index, result.index - index));
          index = result.index + 1;

        }
        open++;
      }
      if (adapter.interpolation.close === char && open > 0) {
        open--;
        if (!open) {
          chunks.push(fn(str.substr(index, result.index - index)));
          index = result.index + 1;
        }
      }
  }
...
exports.hasInterpolation = function(str) {
  return ~str.indexOf(adapter.interpolation.start);
};
